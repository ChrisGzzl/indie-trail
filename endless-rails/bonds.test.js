...
blueEdge.run('state.weaponClocks.command=Infinity;updateArsenal(.2);');
assert.equal(blueEdge.run('state.weaponFx.filter(f=>f.kind==="bondLaser").length'),1,'optical afterglow lasts beyond the previous 0.18-second shot');
assert.equal(blueEdge.run('state.bondStats.blue.damage'),6,'lingering light does not damage targets again');
blueEdge.run('updateArsenal(.16);');
assert.equal(blueEdge.run('state.weaponFx.filter(f=>f.kind==="bondLaser").length'),0,'afterglow is removed at 0.36 seconds');
